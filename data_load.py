...
def validate_row_data(row_data):
    for row in row_data[1:]:
        if row_data.count(row) > 1:
            print("Warning : There is some duplicate data\t{row}")
            return 1
        full_row = ''.join(row)
        if not full_row.isnumeric():
            print("Warning: There is a non numeric\t{row}")
            return 2
        print("Info: Successfully validated data")
    return 0
...
def load_and_filter_data_opeartion(cursor,dataset_file_name):
    ...
    # error_code = validate_row_data(row_data)
    # if error_code == 1:
    #     return 1
    # elif error_code == 2:
    #     return 2
